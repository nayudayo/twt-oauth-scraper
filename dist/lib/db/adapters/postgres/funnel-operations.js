...
exports.PostgresFunnelOperations = void 0;
const errors_1 = require("../errors");
class PostgresFunnelOperations {
    constructor(pool) {
        this.pool = pool;
    }
    async saveFunnelProgress(progress) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO funnel_progress (
    ...
    async updateFunnelProgress(userId, data) {
        const client = await this.pool.connect();
        try {
            // First, check if the user has a funnel progress entry
            const existingProgress = await client.query('SELECT * FROM funnel_progress WHERE user_id = $1', [userId]);
            ...
            if (data.responses) {
                ...
                const newResponses = Object.assign(Object.assign({}, currentResponses), data.responses);
            ...
            await client.query(`UPDATE funnel_progress 
         SET ${updates.join(', ')}
    ...
    async markFunnelComplete(completion) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO funnel_completion (
          user_id, completed_at, completion_data
        ) VALUES ($1, CURRENT_TIMESTAMP, $2)
        ON CONFLICT (user_id) DO UPDATE SET
          completion_data = EXCLUDED.completion_data,
          completed_at = CURRENT_TIMESTAMP`, [completion.user_id, completion.completion_data]);
        }
        catch (error) {
            if (this.isPostgresError(error)) {
                throw errors_1.DatabaseError.fromPgError(error);
    ...
    async getFunnelProgress(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM funnel_progress WHERE user_id = $1', [userId]);
            if (!result.rows[0])
                return null;
    ...
    async getFunnelCompletion(userId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM funnel_completion WHERE user_id = $1', [userId]);
...
exports.PostgresFunnelOperations = PostgresFunnelOperations;
